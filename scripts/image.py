...
def cli() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=str,
        default="all",
        choices=["eva", "avatar", "all"],
        help="The service(s) to build the image for",
    )
    parser.add_argument(
        "--image-tag",
        type=str,
    ...
    parser.add_argument(
        "--no-cache",
        action="store_true",
        help="Do not use cache when building the image(s).",
    )
    parser.add_argument(
        "--push",
        action="store_true",
        help="Push the built image(s).",
    )
    return parser
...
def get_image_names(image_name: str, service_name: str) -> list[str]:
    """Get the image names to build.
    ...
    service_name : str
        The service(s) to build the image for.

    Returns
    -------
    list[str]
        The list of image names to build.
    """
    if service_name == "all":
        return ["thingenious/sermas-eva", "thingenious/sermas-avatar"]
    return (
        [image_name] if image_name else [f"thingenious/sermas-{service_name}"]
    )
...
def main() -> None:
    ...
    image_names = get_image_names(args.image_name, args.service_name)
    allow_error = check_other_platform(container_command, platform_arg)
    for image_name in image_names:
        container_file_prefix = (
            image_name.split("/")[-1].split("-")[-1].capitalize()
        )
        container_file = f"{container_file_prefix}.Containerfile"
        try:
            build_image(
